...
# CONCONEXIÓN AL ELEFANTE DE POSTGRESQL
def init_db():
...
def obtener_nodos():
    ...
    if db_url:
        try:
            ...
            pass
    return pd.DataFrame([{"lat": 40.7128, "lon": -74.0060, "nombre_nodo": "Nodo Central US"}])
...
def obtener_logs():
    ...
    if db_url:
        try:
            ...
            pass
    return pd.DataFrame([{"Fecha/Hora": time.strftime("%Y-%m-%d %H:%M:%S"), "Operador": "Sistema", "Acción": "Modo local b."}])
...
st.markdown(f'<div class="brand-container"><h1 class="brand-title">⚡ T&B Global</h1><p class="brand-subtitle">{L["title"]}</p></div>', unsafe_allow_html=True)

# LOGIN CENTRALIZADO FIJO EN LA PANTALLA PRINCIPAL
if st.session_state["usuario_activo"] is None:
    st.warning(L["lock"])
    st.markdown("### 🔐 Validación de Credenciales de Operador")
    u = st.text_input(f"{L['user']}:")
    p = st.text_input(f"{L['pin']}:", type="password", max_chars=4)
    if st.button("🚀 INGRESAR AL SISTEMA OPERATIVO"):
        if u and p:
...
# VISTA PRINCIPAL INTEGRADA TRAS VALIDACIÓN EXITOSA
df_nodos = obtener_nodos()
df_logs = obtener_logs()

# SECCIÓN MAESTRA DE DOCUMENTOS AL FRENTE
...
with col_d1:
    st.download_button(label="📄 Descargar Acta Legal (TXT)", data=reporte_texto, file_name="documento_legal_tb_global.txt")
with col_d2:
    if st.button("🔒 Cerrar Sesión"):
        guardar_log(st.session_state["usuario_activo"], "Sesión cerrada por el operador.")
