...
# CONTROL DE ACCESO INSTITUCIONAL (Bloqueo Plano)
    ...
    with st.sidebar:
        ...
        if st.button("Validar Credenciales"):
            if u and p:
                ...
                st.rerun()
    st.stop()  # Detiene el script aquí de forma segura si no está logueado

# CONSTRUCCIÓN DE LA BARRA LATERAL (BÓVEDA DE DOCUMENTOS CORPORATIVOS)
with st.sidebar:
...
# ---- DISTRIBUCIÓN EN PARALELO (GRID DE DOS COLUMNAS DE ALTA DENSIDAD VISUAL) ----
col_izquierda, col_derecha = st.columns([1.1, 0.9], gap="large")

# COLUMNA IZQUIERDA: ÁREA 1 (TELEMETRÍA Y CONTROLES GEOGRÁFICOS)
with col_izquierda:
