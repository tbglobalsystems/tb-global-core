...
if st.session_state["usuario_activo"] is None:
    ...
    with st.sidebar:
        ...
        # 📂 REINTEGRACIÓN: BÓVEDA DE DOCUMENTOS INTEGRADA DIRECTO PARA EL PROPIETARIO
        st.markdown("---")
    ...
    # ---- CONSOLA DE CAPACIDAD COMPLETA OPERATIVA (SÓLO CUANDO ESTÁ AUTENTICADO) ----
    
    # ÁREA 1: TELEMETRÍA Y MAPA DINÁMICO
    st.markdown(f"#### {L['area1']}")
    st.map(st.session_state["nodos"], zoom=1, use_container_width=True)
    
    # CONTROLES DE ENFOQUE DEL MAPA (CON ESTILOS MAESTROS AZULES EN PARALELO)
