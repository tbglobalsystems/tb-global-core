...
<style>
    ...
    .brand-subtitle { font-size: 13px !important; color: #ffffff !important; font-weight: 600 !important; letter-spacing: 5px !important; margin-top: 8px !important; }
...
# CONTROL DE ACCESO INSTITUCIONAL (ESTRICTO)
    ...
    with st.sidebar:
        ...
        if st.button("Validar Credenciales"):
            if u and p:
                ...
                st.rerun()
    st.stop()  # Detiene la ejecución aquí si no hay sesión activa

# BARRA LATERAL DEL OPERADOR ACTIVO
with st.sidebar:
...
# ---- CONSOLA DE CAPACIDAD COMPLETA OPERATIVA GLOBAL (EJECUCIÓN PLANA CONTINUA) ----
...
with st.expander(L["open_c"], expanded=False):
    ...
    if btn_nodo:
        if n_name.strip() != "":
            ...
            st.session_state["nodos"] = pd.concat([st.session_state["nodos"], nuevo_registro], ignore_index=True)
