...
# INICIALIZACIÓN DEESTRUCTURAS EN MEMORIA LOCAL DE RESPALDO (CAPA 1)
...
if "logs_memoria" not in st.session_state:
    st.session_state["logs_memoria"] = [
        {"Fecha/Hora": time.strftime("%Y-%m-%d %H:%M:%S"), "Operador": "Sistema", "Acción": "Infraestructura Quantum inicializada en RAM local."}
    ]

# CONEXIÓN SEGURA AL ELEFANTE DE POSTGRESQL (CAPA 2)
def init_db():
    ...
    if db_url:
        try:
            ...
            pass # Tolerancia total a fallos para que nunca detenga la app
    ...
    if db_url:
        try:
            ...
            if not df.empty and 'lat' in df.columns and 'lon' in df.columns:
                # Sincroniza la memoria RAM local con lo que tiene la base de datos
                st.session_state["nodos_memoria"] = df.to_dict(orient="records")
                return df
        except:
            pass
    # Si Postgres no responde, usa el respaldo en memoria RAM local directamente
...
def obtener_logs():
    ...
    if db_url:
        try:
            ...
            pass
    # Si Postgres falla, devolvemos los datos acumulados en la RAM local
    return pd.DataFrame(st.session_state["logs_memoria"])

def guardar_nodo(lat, lon, nombre):
    # Guardado inmediato en memoria RAM local (Capa 1)
    st.session_state["nodos_memoria"].append({"lat": lat, "lon": lon, "nombre_nodo": nombre})
    
    # Guardado secundario en base de datos PostgreSQL (Capa 2)
    db_url = os.environ.get("DATABASE_URL")
    if db_url:
        try:
            conn = psycopg2.connect(db_url, connect_timeout=3)
