...
# CONTROL DE ACCESO INSTITUCIONAL (EVITA BLOQUEOS LÓGICOS DE CORTES)
    ...
    with st.sidebar:
        ...
        if st.button("Validar Credenciales"):
            if u and p:
                ...
                st.rerun()
    st.stop()  # Detiene el flujo de forma segura si no está logueado
...
# ---- DISEÑO DE MATRIZ MAESTRA EN PARALELO (OBLIGA A RENDERIZAR EN UNA SOLA INTERFAZ) ----
container_principal = st.container()

with container_principal:
    col_izquierda, col_derecha = st.columns([1.0, 1.0], gap="large")

    # COLUMNA IZQUIERDA: ÁREA 1 COMPLETA
    with col_izquierda:
