...
# Inicialización plana de variables de idioma
if "lang" not in st.session_state: 
    st.session_state["lang"] = "es"

L = IDIOMAS[st.session_state["lang"]]

# Datos estáticos puros para evitar conflictos de hilos y bloqueos en Railway
...
DATOS_LOGS = pd.DataFrame([
    {"Fecha/Hora": time.strftime("%Y-%m-%d %H:%M:%S"), "Operador": "admin_tb", "Acción": "Inicio de sesión de operador exitoso."},
...
# BÓVEDA DE DOCUMENTOS EN LA BARRA LATERAL FIXED
with st.sidebar:
    st.success("Operador Activo: admin_tb")
    st.markdown("---")
...
# ---- INTERFAZ PLANO-SECUENCIAL DE CAPACIDAD COMPLETA OBLIGATORIA ----

# ÁREA 1: TELEMETRÍA Y MAPA DINÁMICO
st.markdown(f"#### {L['area1']}")
st.map(DATOS_NODOS, zoom=1, use_container_width=True)
...
if btn_nodo:
    st.toast("Aprovisionamiento simulado exitosamente en red principal.", icon="⚡")

# ÁREA 2: INTELIGENCIA VIRTUAL Y MONITOR DE TRÁFICO
st.markdown("---")
st.markdown(f"#### {L['area2']}")
opciones = [L["ana1"], L["ana2"], L["ana3"]]
enfoque_ia = st.radio(L["select_a"], opciones, horizontal=True)
...
with col_t2: 
    st.metric(label=L["m2"], value="135 Gbps")
with col_t3: 
    st.metric(label=L["m3"], value="18 ms")

# ÁREA 3: AUDITORÍA HISTÓRICA E INSTITUCIONAL
st.markdown("---")
st.markdown(f"#### {L['area3']}")
st.dataframe(DATOS_LOGS, use_container_width=True, height=200)

csv_data = DATOS_LOGS.to_csv(index=False).encode('utf-8')
st.download_button(
