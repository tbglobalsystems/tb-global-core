...
# Base de datos local volátil (reemplazable por conexión PostgreSQL remota)
...
# CONTROL DE ACCESO INSTITUCIONAL CRIPTOGRÁFICO RESTRINGIDO
if st.session_state["usuario_activo"] is None:
    ...
    with st.sidebar:
        ...
        if st.button("Validar Credenciales"):
            # RESTRICCIÓN DE SEGURIDAD EXCLUSIVA PARA EL CREADOR
            if u == "admin_tb" and p == "1234":
                st.session_state["usuario_activo"] = u
                st.rerun()
            else:
                st.error("❌ Credenciales inválidas o denegadas por el sistema principal.")
    st.stop()
...
# ---- INTERFAZ PLANO-SECUENCIAL DE ALTA CAPACIDAD ----
...
# CONSOLA PARA REGISTRAR NUEVOS SERVIDORES EN TIEMPO REAL
st.markdown("---")
with st.expander(L["open_c"], expanded=False):
    ...
    if btn_nodo:
        if n_name.strip() != "":
            ...
            st.session_state["logs_db"] = pd.concat([nuevo_log, st.session_state["logs_db"]], ignore_index=True)
            st.success(f"✔️ '{n_name}' {L['success_reg']}")
            time.sleep(0.5)
