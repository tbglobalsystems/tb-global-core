...
# CONTROL DE ACCESO ORIGINAL DE LA PLATAFORMA (CORREGIDO L_LOCK MAESTRO)
if st.session_state["usuario_activo"] is None:
    ...
    <div class="brand-container">
        ...
        <p class="brand-subtitle">{L_lock['title']}</p>
    </div>
    """, unsafe_allow_html=True)
    
    st.warning(L_lock["lock"])
    with st.sidebar:
        st.markdown("### 🔐 Acceso Centralizado")
        u = st.text_input(f"{L_lock['user']}:")
        p = st.text_input(f"{L_lock['pin']}:", type="password", max_chars=4)
        if st.button("Validar Credenciales"):
            if u and p:
                ...
                st.session_state["logs_data"] = pd.concat([log_login, st.session_state["logs_data"]], ignore_index=True)
                st.rerun()
    st.stop()
else:
    ...
    with st.sidebar:
        ...
        if st.button("🔒 Cerrar Sesión"):
            ...
            st.session_state["logs_data"] = pd.concat([log_logout, st.session_state["logs_data"]], ignore_index=True)
...
st.markdown("---")
# CONSOLA DE REGISTRO INTEGRAL CONECTADA
...
if btn_nodo:
    if n_name.strip() != "":
        ...
        nuevo_log = pd.DataFrame([{"Fecha/Hora": time.strftime("%Y-%m-%d %H:%M:%S"), "Operador": st.session_state["usuario_activo"], "Acción": f"Aprovisionó Servidor Canal: {n_name} ({n_lat}, {n_lon})"}])
