...
# FUNCIÓN MAESTRA: CONEXIÓN AL ELEFANTE DE POSTGRESQL
def init_db():
    ...
    if db_url:
        try:
            ...
            cursor = conn.cursor()
            # Crear tablas en el elefante si no existen
            cursor.execute("""
                CREATE TABLE IF NOT EXISTS nodos (
                    id SERIAL PRIMARY KEY,
                    lat DOUBLE PRECISION,
                    lon DOUBLE PRECISION,
                    nombre_nodo VARCHAR(255)
                );
            """)
            cursor.execute("""
            ...
            """)
            # Insertar datos base si las tablas están recién creadas y vacías
            ...
            st.error(f"Error de base de datos remota: {e}")

# Ejecutar inicialización de tablas permanentes
init_db()

# FUNCIONES DE CARGA Y GUARDADO REAL
def obtener_nodos():
    db_url = os.environ.get("DATABASE_URL")
    if db_url:
        try:
            ...
            pass
    # Respaldo volátil si Postgres falla
    if "nodos" not in st.session_state:
        st.session_state["nodos"] = pd.DataFrame([{"lat": 40.7128, "lon": -74.0060, "nombre_nodo": "Nodo Central US"}])
    return st.session_state["nodos"]
...
def obtener_logs():
    ...
    if "logs" not in st.session_state:
        st.session_state["logs"] = pd.DataFrame([{"Fecha/Hora": time.strftime("%Y-%m-%d %H:%M:%S"), "Operador": "Sistema", "Acción": "Modo volátil activo."}])
...
def guardar_nodo(lat, lon, nombre):
    ...
    if db_url:
        try:
            ...
            pass
    st.session_state["nodos"] = pd.concat([st.session_state["nodos"], pd.DataFrame([{"lat": lat, "lon": lon, "nombre_nodo": nombre}])], ignore_index=True)
...
def guardar_log(operador, accion):
    ...
    if db_url:
        try:
            ...
            pass
    nuevo_df = pd.DataFrame([{"Fecha/Hora": time.strftime("%Y-%m-%d %H:%M:%S"), "Operador": operador, "Acción": accion}])
    st.session_state["logs"] = pd.concat([nuevo_df, st.session_state["logs"]], ignore_index=True)
...
# CONTROL DE ACCESO ORIGINAL DE LA PLATAFORMA
if st.session_state["usuario_activo"] is None:
    L_lock = IDIOMAS[st.session_state["lang"]]
